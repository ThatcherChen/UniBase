...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
    ...
    int curOffset = lseek(fd, page_no * PAGE_SIZE, SEEK_SET);
    if (curOffset == -1) {
        throw UnixError();
    }
    int wrotenBytes = write(fd, offset, num_bytes);
    if (wrotenBytes != num_bytes) {
...
void DiskManager::read_page(int fd, page_id_t page_no, char *offset, int num_bytes) {
    ...
    int readBytes = read(fd, offset, num_bytes);
